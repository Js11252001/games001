...
def solve_constraints():
    ...
    p_center = ti.Vector([0.0, 0.0, 0.0])
    q_center = ti.Vector([0.0, 0.0, 0.0])
    for i in range(vertices_num):
        p_center += rest_vertices[i]
        q_center += cur_vertices[i]
    p_center /= vertices_num
    q_center /= vertices_num


    A = ti.Matrix.zero(ti.f32, 3, 3)
    for i in range(vertices_num):
        dp = rest_vertices[i] - p_center
        dq = cur_vertices[i] - q_center
        A += dq.outer_product(dp)

    U, S, V = ti.svd(A)
    R = U @ V.transpose()  # 最优旋转矩阵
